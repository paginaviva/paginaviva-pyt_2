...
/**
 * phase_2c_proxy.php
 * Proxy para Fase 2C: Añadir campos taxonómicos usando Assistants API
 * 
...
class Phase2CProxy
{
    ...
    public function execute(array $input): array
    {
        ...
        return [
            'output' => [
                'tex' => "\xEF\xBB\xBFClasificación taxonómica completada",
    ...
    private function validateInput(): void
    {
        ...
        $this->fileIdTaxonomia = $this->config['file_id_taxonomia'] ?? '';
        if (empty($this->fileIdTaxonomia)) {
            $this->fail(500, 'file_id_taxonomia no configurado en config.json');
        }
        
        // Verificar que existe el JSON de F2B
        $jsonFile = $this->getJsonPath();
        if (!file_exists($jsonFile)) {
            $this->fail(400, 'Debe completar Fase 2B primero. No se encontró: ' . basename($jsonFile));
        }
        
        // Leer JSON previo de F2B (14 campos)
        $jsonContent = file_get_contents($jsonFile);
        $this->jsonPrevio = json_decode($jsonContent, true);
        
        if (!is_array($this->jsonPrevio)) {
            $this->fail(400, 'El JSON de F2B no es válido');
        }
        
        // Validar que tiene al menos los campos básicos de F2B
        $requiredKeys = ['file_id', 'nombre_producto', 'codigo_referencia_cofem', 'uso_formacion_tecnicos'];
        foreach ($requiredKeys as $key) {
            if (!array_key_exists($key, $this->jsonPrevio)) {
                $this->fail(400, 'El JSON de F2B no tiene el campo requerido: ' . $key);
            }
        }
    ...
    private function runAssistantAnalysis(): array
    {
        ...
        // Agregar mensaje con referencia a ambos archivos y JSON previo
    ...
    private function getOrCreateAssistant(string $apiKey, ?string $modelOverride = null): string
    {
        ...
        // Verificar si existe assistant_id guardado para F2C
        ...
        if (!$promptTemplate) {
            $this->fail(500, 'Prompt de F2C no encontrado en config/prompts.php');
        }
    ...
    private function saveResults(array $jsonResult): void
    {
        ...
        // Guardar JSON final (sobrescribe el de F2B)
        ...
        $logPath = $docDir . DIRECTORY_SEPARATOR . $this->docBasename . '_2C.log';
        $logData = [
    ...
    {
        $docsDir = $this->config['docs_dir'] ?? '';
        return $docsDir . DIRECTORY_SEPARATOR . $this->docBasename . DIRECTORY_SEPARATOR . $this->docBasename . '_2C.assistant_id';
    }
    ...
    private function getFilePaths(): array
    {
        ...
        return [
            ...
            'log_file' => $docDir . DIRECTORY_SEPARATOR . $this->docBasename . '_2C.log',
            'assistant_id_file' => $docDir . DIRECTORY_SEPARATOR . $this->docBasename . '_2C.assistant_id'
        ];
    }
...
$proxy = new Phase2CProxy($docBasename, $input);
